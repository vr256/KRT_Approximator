...
def calculate_y():
    ...
    if way and CURRENT_METHOD in ["Псевдооберненої матриці", "Генетичний алгоритм"]:
        ...
        res_y, res_lam, res_a, res_c = main_solution(
            ...
            polynom=CURRENT_POLYNOM,
            degs=(x1_deg, x2_deg, x3_deg),
        )
        Y_DATA_PRED = res_y
        make_plots()
        total = ""
        ...
        if not LATEX:
            ...
            for ind, coef in enumerate(lam):
                ...
                for vec_id, x_vec in enumerate(coef):
                    for elem_id, x_elem in enumerate(x_vec):
                        for deg, val in enumerate(x_elem.numpy()):
                            res += rf"{val:.4f} \cdot \ln \left ( {pol_name}_{{{deg}}}(x_{{{vec_id + 1}{elem_id + 1}}}) 1 \right ) "
                res = re.sub(r"( )(\d)", r"\1+ \2", res, count=len(res.split(" ")))
                res = res.replace("=", "= ").replace("-", "- ").rstrip()
                total += res + "\n\n"
    else:
