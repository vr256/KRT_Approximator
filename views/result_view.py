...
from tools.config import PATH_DARK, PATH_LIGHT, Locale, Theme
...
class Sidebar(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.logo_label = customtkinter.CTkLabel(
            self, text=self.loc['header'],
            font=customtkinter.CTkFont(size=18, weight='bold'))
        self.logo_label.grid(row=0, column=0, padx=10, pady=(20, 10))
        self.info_label = customtkinter.CTkLabel(
            self, text=self.loc['info'], anchor='nw')
        self.info_label.grid(row=1, column=0, columnspan=2, padx=10, pady=(10, 0))

        self.locale_label = customtkinter.CTkLabel(
            self, text=self.loc['locale_caption'], anchor='w')
        self.locale_label.grid(row=3, column=0, padx=10, pady=(10, 0), sticky='w')
        self.locale_optionemenu = customtkinter.CTkOptionMenu(self, values=self.loc['locales'],
                                                              command=change_locale)
        self.locale_optionemenu.grid(
            row=4, column=0, padx=10, pady=(0, 10))

        self.appearance_mode_label = customtkinter.CTkLabel(
            self, text=self.loc['theme_caption'], anchor='w')
        self.appearance_mode_label.grid(row=5, column=0, padx=10, pady=(10, 0), sticky='w')
        self.appearance_mode_optionemenu = customtkinter.CTkOptionMenu(self, values=self.loc['themes'],
...
class YCalculator(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.calculate_y_button = customtkinter.CTkButton(self, text=self.loc['find_approx'],
                                                          command=calculate_y)
...
class MainTabview(customtkinter.CTkTabview):
    def __init__(self, master, **kwargs):
        ...
        self.results_plot = customtkinter.CTkLabel(
            self.tab(self.plot_tab_caption), text='', image=my_image)
