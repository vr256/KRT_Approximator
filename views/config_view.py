...
from tools.config import SEARCH_ICON, Polynomial
...
class Optimizer(customtkinter.CTkFrame):
    def __init__(self, master, **kwargs):
        ...
        self.opt_system_method = customtkinter.CTkOptionMenu(self, values=self.loc['optimizers'],
                                                             command=change_method)
        self.opt_system_method.grid(row=2, column=2, padx=10, pady=5)

    def update_locale(self):
        self.loc = load_locale(current_module)
        self.label_system_solution_selector.configure(text=self.loc['optimizer_caption'])
...
class PolynomView(customtkinter.CTkScrollableFrame):
    def __init__(self, master, **kwargs):
        ...
        self.opt_polynom = customtkinter.CTkOptionMenu(self, values=self.loc['polynomials'],
        ...
        for i in range(1, 3 + 1):
            ...
            self.__dict__[f'label_X{i}_deg'] = customtkinter.CTkLabel(
                self, text=self.loc['pow'] + f' X{i}:')
            self.__dict__[f'label_X{i}_deg'].grid(row=1 + i, column=0, padx=(0, 5), pady=10)
            self.__dict__[f'entry_X{i}_deg'] = customtkinter.CTkEntry(
                self, width=40, textvariable=self.__dict__[f'X{i}_deg'])
            self.__dict__[f'entry_X{i}_deg'].grid(row=1 + i, column=1, pady=10, sticky='w')
